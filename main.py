...
def heroi_pronto_por_nome(nomeHeroi):
    ...
    itensHeroi = itens_do_heroi.itens_em_uso_por_nome_do_heroi(nomeHeroi)
    itensEmUso = []

    for item in itensHeroi:
        if item['emUso'] == 1:
            itensEmUso.append(item)

    for skill in itensEmUso:
        heroi['fisico'] += skill['fisico']
        heroi['magia'] += skill['magia']
        heroi['agilidade'] += skill['agilidade']
...
def atacar_com_fisico(atacante, defensor):
    ...
    attk = int(atacante['agilidade'] / defensor['agilidade'])

    if attk < 1:
        attk = 1

    for _ in range(0, attk): 
...
# select * from 
# ItemDoHeroi JOIN Item
# on Item.id = ItemDoHeroi.idItem
# where ItemDoHeroi.idHeroi = 1;
...
# '''
def lista_itens_em_uso_do_heroi(id):
    itens = itens_do_heroi.itens_do_heroi(id)
...
def criar_heroi(nome,agilidade,fisico,magia):
    ...
    herois.criar_novo_heroi(nome, agilidade, fisico, magia)
